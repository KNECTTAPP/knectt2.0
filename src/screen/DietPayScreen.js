...
const DietPayScreen = ({ navigation }) => {
  ...
  const createOder = async () => {
    ...
    try {
      ...
      if (json.status == 200) {
        ...
        const responsepayment = await fetch(EndUrl.updatepaymentid, {
          method: "POST",
          headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
        ...
        if (jsonpayment.plan_purchased == 1) {
          navigation.replace("DietThanks", {
            plan_type: jsonpayment.plan_type,
          });
        } else {
    ...
    <SafeAreaView style={styles.screenContainer}>
      ...
      >
        {cartproduct.length > 0 ? (
          ...
          // uppercase={false} title="Checkout & Pay" onPress={() => createOder()} />
          <ButtonCustom title="Checkout & Pay" onPress={() => createOder()} />
        ) : null}
