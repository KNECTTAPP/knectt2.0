  ...
  return (
    ...
    <View style={styles.container}>
      ...
      {/* {userId ? ( */}
        <WebView
          originWhitelist={['*']}
          source={{ uri:`https://knectt-ai-tracking.lovable.app/?customerId=${userId}` }}
          ...
          onMessage={(event) => {
            ...
            //     handleEnd();
            //   } else {
                // Optional: navigate back if no handleEnd callback provided
                navigation?.goBack?.();
            //   }
            }
            else if (event.nativeEvent.data === "loginApp") {
        navigation.reset({ routes: [{ name: "Ulogin" }] });
        return;
      }
          }}
        />
